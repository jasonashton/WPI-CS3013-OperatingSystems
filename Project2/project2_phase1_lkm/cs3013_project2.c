...
asmlinkage long new_sys_read(int fd, void *buf, size_t count) {
  int bytesRead; // Holds the return value of the function
  bytesRead = ref_sys_read(fd, buf, count);
  // Now process the read
  if( current_uid().val >= 1000 ) {
    // This is when we trap the call
    if(strstr(buf, "VIRUS") == NULL) {
      // We didn't find a virus

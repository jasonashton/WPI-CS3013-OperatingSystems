// For pthread_create(), pthread_join(), pthread_mutex_lock_...()
#include <pthread.h>
// For sleep()
#include <unistd.h>
...
void *plane_invoke_function(void *args) {
  ...
  if( needsEmergencyInFuture ) { 
    ...
    //  OUR CUT-OFF ATTEMPT
    // Grab the mutex_lock lock and airplane lock
    ...
    //  MUTEX: Release the mutex_lock for airplane lock
    pthread_mutex_unlock(airplane_lock + airplane_index);
    //  MUTEX CODE BLOCK END
	...
	//  MUTEX: Grab the airplane lock
	pthread_mutex_lock(airplane_lock + ctr);
	//  MUTEX CODE BLOCK END

	all_airplanes[ctr].priority += 1; // Bump up the priority by one

	// Release the airplane lock
	//  MUTEX: Release the airplane lock
	pthread_mutex_unlock(airplane_lock + ctr);
...
void planes_reset_priority() {
  ...
  for( ; airplane_loopctr < AIRPLANES_AIRSPACE; airplane_loopctr++ ) {
    ...
    // Grab the mutex for the current airplane as we could be making changes to the airplanes properties
    //  MUTEX: Grab the airplane lock for the current airplane
    pthread_mutex_lock(airplane_lock + airplane_loopctr);
    //  MUTEX CODE BLOCK END
    ...
    //  MUTEX: Release the airplane lock for the current airplane
    pthread_mutex_unlock(airplane_lock + airplane_loopctr);
    //  MUTEX CODE BLOCK END
  }
...
void planes_reorder() {
  ...
  for( ctr = 0; ctr < nonEmergencyCount; ctr++ ) {
    ...
    //  MUTEX: Grab the airplane lock
    pthread_mutex_lock(airplane_lock + planeIndex);
    //  MUTEX CODE BLOCK END
    ...
    // Release the mutex lock for the current airplane
    //  MUTEX: Release the airplane lock
    pthread_mutex_unlock(airplane_lock + planeIndex);
    //  MUTEX CODE BLOCK END
  }
...
int plane_resolve_runway_available() {
  ...
  int *tempval = (int*)malloc(sizeof(int)); // Will hold the mutex's value
  for( ; ctr < RUNWAYS_AIRSPACE; ctr++ ) {
    *tempval = runwaysLocked[ctr]; // Getting if the runway is locked or not 
    if( *tempval == 0 ) {
...
void plane_land_sequence(int airplane_index, int runwayToLandOn, int airplaneID) {
  ...
  //  MUTEX: Lock on the runway that we ant to land on
  pthread_mutex_lock(runway_lock + runwayToLandOn);
  //  MUTEX CODE BLOCK END
  ...
  for( ; ctr < AIRPLANES_AIRSPACE; ctr++ ) {
    if( all_airplanes[ctr].in_airspace && all_airplanes[ctr].priority != 0 ) {
      ...
      //  MUTEX: Grab the airplane lock
      pthread_mutex_lock(airplane_lock + ctr);
      //  MUTEX CODE BLOCK ENDS

      all_airplanes[ctr].priority--;
      
      // Release the mutex lock on the airplane now
      //  MUTEX: Release the airplane lock
      pthread_mutex_unlock(airplane_lock + ctr);
      //  MUTEX CODE BLOCK ENDS
    }
  ...
  runwaysLocked[runwayToLandOn] = 0; // We no longer have this runway locked
  ...
  //  MUTEX: Unlock the runway for it to be used by other airplanes
  pthread_mutex_unlock(runway_lock + runwayToLandOn);
  //  MUTEX CODE BLOCK END
}
...
int main(int argc, char **argv) {
  ...
  for( ctr = 0; ctr < RUNWAYS_AIRSPACE; ctr++ )
    pthread_mutex_init(runway_lock + ctr, 0); // Initialize the runway lock as well 
  ...
  // Initialize all the values for runways locked
  for( ctr = 0; ctr < RUNWAYS_AIRSPACE; ctr++ )
    runwaysLocked[ctr] = 0; // None of the runways are locked initially

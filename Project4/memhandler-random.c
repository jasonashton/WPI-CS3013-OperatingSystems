...
vAddr create_page() {
  ...
  // Move onto incrementing the overall memory used
  overallMemoryUsed++;
...
int *pickAPageToEvict(int level) {
  ...
  if( level == 0 ) {
    ...
    int ctr = 0; // Loop Counter
    int firstTry = 0; // First try boolean
    for( ; ctr < RAM_STORAGE_COUNT; ctr++ ) {
      if( ramStoredHere[ctr] == 0 ) {
        // Page exists
        if( limitedRandom(0, MAXIMUM_RANDOM) < THRESHOLD_RANDOM ) {
          // We've found a page that exists and that can be randomly evicted
	  toReturn[1] = ctr;
	  break; // Exit the loop as we found are match
	}
	// If the condition didn't match and it was the first try
	if( !firstTry ) {
          firstTry = 1; // First try has been exhaused
	  toReturn[1] = ctr;
	}
      }
    }
  ...
  // Tell the user
  if( level == 0 )
    printf("Random Page Picking algorithm dictates eviction of page with #%d from the RAM level\n", getPageTableNumber(0, toReturn[1]));
  else if( level == 1 )
    printf("Random Page Picking algorithm dictates eviction of page with #%d from the SSD level\n", getPageTableNumber(1, toReturn[1]));
  else if( level == 2 )
    printf("Random Page Picking algorithm dictates eviction of page with #%d from the HDD level\n", getPageTableNumber(2, toReturn[1]));

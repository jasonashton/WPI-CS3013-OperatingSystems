...
typedef struct {
  ...
  int counter; // Counter to keep track of when a page entered in the FIFO model
} PageTableEntry;
...
vAddr create_page() {
  ...
  pageTable[pageTableIndex].presentBit = 1; // We are now adding a page
  ...
  pageTable[pageTableIndex].counter = 0; // FIFO counter = 0 => Freshly come in page
  // Increment counters on each page other than this one
  for( ctr = 0; ctr < overallMemoryUsed; ctr++ )
    if( pageTable[ctr].presentBit == 0 ) // If the page exists, then increment its counter
      pageTable[ctr].counter++; // Increment the counter as the FIFO counter is one later than the page that newly came in
  // Finally, move onto incrementing the overall memory used
  overallMemoryUsed++;
...
void store_value(vAddr address, uint32_t *value) {
  ...
  for( ; ctr < TOTAL_SYSTEM_MEM_STORAGE; ctr++ ) {
    if( pageTable[ctr].pageNumber == address && pageTable[ctr].presentBit ) {
      // We've found the right page. Look at its level
      if( pageTable[ctr].level == 0 ) {
...
void free_page(vAddr address) {
  ...
  for( ; ctr < TOTAL_SYSTEM_MEM_STORAGE; ctr++ ) {
    if( pageTable[ctr].pageNumber == address ) {
      ...
      pageTable[ctr].counter = -1; // This page is no longer in the FIFO sequence giving it a counter of -1
...
void __initialize() {
  ...
  for( ctr = 0; ctr < TOTAL_SYSTEM_MEM_STORAGE; ctr++ ) {
    ...
    pageTable[ctr].counter = -1; // Not yet in the FIFO sequence, so we set as -1
  }
...
int *pickAPageToEvict(int level) {
  ...
  if( level == 0 ) {
    ...
    int ctr = 0; // Loop Counter
    int maximumFIFOCounter = -99999, maximumIndex = 0; // Maximum FIFO counter is minimum by default and index in RAM of that
                                                      //  max FIFO counter is zero.
    for( ; ctr < TOTAL_SYSTEM_MEM_STORAGE; ctr++ ) {
	...
	// Now, we compare to find a maximum:
	if( pageTable[ctr].counter > maximumFIFOCounter ) {
          // We found a new maximum:
	  maximumFIFOCounter = pageTable[ctr].counter;
	  maximumIndex = pageTable[ctr].index;
	  // Save a new one:
	  toReturn[1] = maximumIndex;
	}
      }
    }
  } else if( level == 1 ) {
  ...
  // Tell the user
  if( level == 0 )
    printf("FIFO Paging algorithm dictates eviction of page with #%d from the RAM level\n", getPageTableNumber(0, toReturn[1]));
  else if( level == 1 )
    printf("FIFO Paging algorithm dictates eviction of page with #%d from the SSD level\n", getPageTableNumber(1, toReturn[1]));
  else if( level == 2 )
    printf("FIFO Paging algorithm dictates eviction of page with #%d from the HDD level\n", getPageTableNumber(2, toReturn[1]));
